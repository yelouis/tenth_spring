...
**M3 — Background capture implemented; the "foreground-only" finding is resolved (July 22, 4th verification pass).**
- **What was solved:** `OsLocationSource` gained `buildLocationSettings()` (`os_location_source.dart:27-55`), branching to `AndroidSettings` (foreground-service notification "Tenth Spring" / "Scouting your map", `enableWakeLock: false`, 2-min interval) and `AppleSettings` (`allowBackgroundLocationUpdates: true`, `pauseLocationUpdatesAutomatically: true`, `showBackgroundLocationIndicator: false`, `activityType: other`). All six Android permissions are declared; iOS carries both usage strings and `UIBackgroundModes`. The battery levers were correctly left untouched (accuracy `medium`, 25 m filter). `main.dart:9` wires the real source into the app, so this is live rather than dead code, and `os_location_source_test.dart` is a genuinely falsifying test (it would fail against the previous bare `LocationSettings`).
- **Promoted to design:** the capture tuning values (25 m distance filter, 2-min interval) previously existed only in code and are now recorded in the master plan Core Configurations and `design_privacy_and_location.md` §2.
- **Not closed:** the device gate has not been run (see D3 status correction and **Decision 5**), and **F8**/**F9** were filed from this pass.
- **DB accessor prep (same commit):** `db.gd:130,133` added `get_base_state()` / `set_player_tile()`, and `relocation_manager.gd` no longer reaches into private stores — the F4 blast radius named in the previous guide is now pre-cleared.
